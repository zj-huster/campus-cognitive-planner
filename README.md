...
# 演示视频
bilibili:
https://t.bilibili.com/1184432562997559352?share_source=pc_native
...
- ⏰ **时间分配**: 智能分配学习时间
- 🧠 **自适应学习画像（MVP）**: 根据任务完成情况在线更新科目效率与时段专注因子，动态优化后续周/日计划
- 🤖 **AI助手**: 与AI对话，获取个性化建议
...
│   │   ├── goal-tree.ts
│   │   ├── intervention.ts
│   │   ├── learner-profile.ts # 学习画像（科目效率/时段专注）
│   │   ├── memory-store.ts
│   │   ├── plan-summary.ts
│   │   ├── read-file.ts
│   │   ├── risk-predict.ts
...
├── data/                     # 数据目录
│   ├── goals.json            # 学习目标
│   ├── learner-profile.json  # 学习画像数据（自动生成）
│   ├── tasks.json            # 学习任务
│   ├── logs/                 # 对话日志
│   └── plans/                # 学习计划
├── package.json
...
1. **状态管理**: 维护学习目标、任务和进度
2. **风险评估**: 基于多个指标评估学习风险
3. **自适应画像更新**: 在任务完成后根据计划/实际时长更新科目效率和时段专注因子
4. **智能调度**: 根据优先级、可用时间与学习画像分配任务
5. **AI决策**: 使用大语言模型进行智能决策
6. **工具调用**: Agent可以调用多种工具完成任务
